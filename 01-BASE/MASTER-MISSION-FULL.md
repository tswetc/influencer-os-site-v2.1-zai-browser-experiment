...
6. Research current models/references/MCP if web is genuinely available.7. Finish shared architecture, real-generation attempt, MCP, auth/data.
...
export function mods(c: CaptureSelection): string {
  ...
  for (const k of c.imperf) if (IMPERF[k]) parts.push(IMPERF[k]);  if (c.film && FILM[c.film]) parts.push(FILM[c.film]);
  return parts.join(", ");
}
...
export function appendVersion(
  ...
  const v: PassportVersion = {    id: `${passport.id}_${Date.now().toString(36)}_${list.length}`,
